...
for i in range(2):
    ...
    intermiate_output = finetune_net.features[:14]
    newNet = nn.HybridSequential()
    with newNet.name_scope():
         newNet.add(
         intermiate_output,
         nn.Conv2D(classes, kernel_size=(1, 1)),
         nn.Activation('relu'),
         nn.AvgPool2D(13, strides=(13, 13), padding=(0, 0)),
         nn.Flatten())
        
    newNet.collect_params('.*sequential').initialize(init.Xavier(), ctx = ctx)
    newNet.collect_params().reset_ctx(ctx)
    newNet.hybridize()
    epochs = 40
    trainer1 = gluon.Trainer(newNet.collect_params(), 'adam') #{'learning_rate': 0.1, 'wd': 1e-6}
    train(train_iter, val_iter, newNet, loss, trainer1, ctx, num_epochs=epochs)
    final_test_acc = evaluate_accuracy(test_iter, newNet, ctx)
    print(model_name, final_test_acc)
    np.save('/rds/project/t2_vol3/rds-t2-cs105/Covid-19/dataAll_5_class/results/_original_squeezenet_40_adam_train_'+str(i)+'.npy', hold_accuracy_train)
    np.save('/rds/project/t2_vol3/rds-t2-cs105/Covid-19/dataAll_5_class/results/_original_squeezenet_40_adam_val_'+str(i)+'.npy', hold_accuracy_val)
    file_name = "/rds/project/t2_vol3/rds-t2-cs105/Covid-19/dataAll_5_class/models/squeezenet"+str(i)+".params"
    newNet.save_parameters(file_name)
